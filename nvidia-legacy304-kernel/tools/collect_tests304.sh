...
DEFFILE=got_defined304.txt
TMPDIR1=tmp304

OUTFILE=compile_tests304.sh
rm -f $OUTFILE
LISTFILE=list_tests304.mk
rm -f $LISTFILE
...
SINCE=`egrep -n '^build_cflags$' conftest.sh | cut -f 1 -d ':'`
SINCE=$(($SINCE - 3))
...
SINCE=`egrep -n '^# Miscellaneous NVIDIA kernel ' Makefile | cut -f 1 -d ':'`
SINCE=$(($SINCE - 1))
